...
export async function extractRecipeIngredients(recipeUrl: string, servings: number = 4): Promise<any[]> {
  console.log(`Extracting ingredients from recipe: ${recipeUrl} for ${servings} servings`);

  try {
    // In production, this would use web scraping or recipe APIs
    // For demo, use OpenAI to generate realistic ingredients based on URL
    ...
    if (process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY !== 'demo-key') {
      try {
        console.log('Using OpenAI to extract recipe ingredients...');

        const prompt = `Given this recipe URL: ${recipeUrl}

        Extract a realistic ingredient list for ${servings} servings. Return ONLY a JSON array in this exact format:
        [
          {"name": "ingredient name", "quantity": number, "unit": "unit"},
          {"name": "ingredient name", "quantity": number, "unit": "unit"}
        ]

        Use standard cooking units like: cup, tablespoon, teaspoon, pound, ounce, clove, piece, etc.
        Make quantities realistic for ${servings} servings.`;
        ...
        if (content) {
          ...
          if (jsonMatch) {
            ...
            if (Array.isArray(parsedResponse) && parsedResponse.length > 0) {
              console.log(`OpenAI extracted ${parsedResponse.length} ingredients`);
              return parsedResponse;
            }
          }
        }
      } catch (openaiError) {
        console.warn('OpenAI extraction failed, falling back to mock data:', openaiError);
      }
    }

    // Fallback to recipe-specific mock data
    console.log('Using recipe-specific mock data');
...
function getRecipeSpecificMockData(recipeUrl: string, servings: number): any[] {
  const urlLower = recipeUrl.toLowerCase();
  const multiplier = servings / 4; // Base recipes assume 4 servings

  console.log(`Generating mock data for URL: ${recipeUrl}, servings: ${servings}, multiplier: ${multiplier}`);

  // Analyze URL for recipe type keywords and return appropriate ingredients
  if (urlLower.includes('chicken') || urlLower.includes('poultry')) {
    return [
      { name: "Chicken breast", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "pounds" },
      { name: "Olive oil", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "tablespoons" },
      { name: "Salt", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "teaspoon" },
      { name: "Black pepper", quantity: Math.max(1, Math.round(0.5 * multiplier)), unit: "teaspoon" },
      { name: "Garlic powder", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "teaspoon" },
      { name: "Onion", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "medium" }
    ];
  } else if (urlLower.includes('beef') || urlLower.includes('steak')) {
    return [
      { name: "Beef", quantity: Math.max(1, Math.round(1.5 * multiplier)), unit: "pounds" },
      { name: "Salt", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "teaspoon" },
      { name: "Black pepper", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "teaspoon" },
      { name: "Garlic", quantity: Math.max(1, Math.round(3 * multiplier)), unit: "cloves" },
      { name: "Butter", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "tablespoons" },
      { name: "Fresh thyme", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "tablespoon" }
    ];
  } else if (urlLower.includes('cake') || urlLower.includes('dessert') || urlLower.includes('baking')) {
    return [
      { name: "All-purpose flour", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "cups" },
      { name: "Sugar", quantity: Math.max(1, Math.round(1.5 * multiplier)), unit: "cups" },
      { name: "Eggs", quantity: Math.max(1, Math.round(3 * multiplier)), unit: "large" },
      { name: "Butter", quantity: Math.max(1, Math.round(0.5 * multiplier)), unit: "cup" },
      { name: "Baking powder", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "teaspoons" },
      { name: "Vanilla extract", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "teaspoon" },
      { name: "Milk", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "cup" }
    ];
  } else if (urlLower.includes('soup') || urlLower.includes('stew')) {
    return [
      { name: "Chicken broth", quantity: Math.max(1, Math.round(4 * multiplier)), unit: "cups" },
      { name: "Onion", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "medium" },
      { name: "Carrots", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "medium" },
      { name: "Celery", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "stalks" },
      { name: "Garlic", quantity: Math.max(1, Math.round(3 * multiplier)), unit: "cloves" },
      { name: "Bay leaves", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "leaves" },
      { name: "Salt", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "teaspoon" },
      { name: "Black pepper", quantity: Math.max(1, Math.round(0.5 * multiplier)), unit: "teaspoon" }
    ];
  } else if (urlLower.includes('pasta')) {
    return [
      { name: "Pasta", quantity: Math.max(1, Math.round(16 * multiplier)), unit: "ounces" },
      { name: "Olive oil", quantity: Math.max(1, Math.round(3 * multiplier)), unit: "tablespoons" },
      { name: "Garlic", quantity: Math.max(1, Math.round(4 * multiplier)), unit: "cloves" },
      { name: "Onion", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "medium" },
      { name: "Tomatoes", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "cans" },
      { name: "Parmesan cheese", quantity: Math.max(1, Math.round(0.5 * multiplier)), unit: "cup" },
      { name: "Fresh basil", quantity: Math.max(1, Math.round(0.25 * multiplier)), unit: "cup" }
    ];
  } else if (urlLower.includes('salad')) {
    return [
      { name: "Mixed greens", quantity: Math.max(1, Math.round(6 * multiplier)), unit: "cups" },
      { name: "Cherry tomatoes", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "cup" },
      { name: "Cucumber", quantity: Math.max(1, Math.round(1 * multiplier)), unit: "medium" },
      { name: "Red onion", quantity: Math.max(1, Math.round(0.25 * multiplier)), unit: "cup" },
      { name: "Olive oil", quantity: Math.max(1, Math.round(3 * multiplier)), unit: "tablespoons" },
      { name: "Lemon juice", quantity: Math.max(1, Math.round(2 * multiplier)), unit: "tablespoons" },
      { name: "Salt", quantity: Math.max(1, Math.round(0.5 * multiplier)), unit: "teaspoon" }
    ];
  }

  // Default fallback for general recipes
  console.log('Using default fallback ingredients');
